...
class McpSyncClientTests {
    ...
    void tearDown() {
        if (mcpSyncClient != null) {
            assertThatCode(() -> mcpSyncClient.close())
                    .doesNotThrowAnyException();
        }
    }
    ...
    void testConstructorWithInvalidArguments() {
        assertThatThrownBy(() -> 
            McpClient.sync(null, TIMEOUT, new ObjectMapper())
        )
        .isInstanceOf(IllegalArgumentException.class);

        assertThatThrownBy(() -> 
            McpClient.sync(new StdioServerTransport(stdioParams), null, new ObjectMapper())
        )
        .isInstanceOf(IllegalArgumentException.class);

        assertThatThrownBy(() -> 
            McpClient.sync(new StdioServerTransport(stdioParams), TIMEOUT, null)
        )
        .isInstanceOf(IllegalArgumentException.class);
    }
    ...
    @Timeout(15)
    void testPing() {
        assertThatCode(() -> mcpSyncClient.ping())
            .doesNotThrowAnyException();
    }
    ...
    void testCallToolWithInvalidTool() {
        ...
        assertThatThrownBy(() -> mcpSyncClient.callTool(invalidRequest))
    ...
    void testRootsListChanged() {
        assertThatCode(() -> mcpSyncClient.sendRootsListChanged())
            .doesNotThrowAnyException();
    }
